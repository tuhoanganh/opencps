...
<%
	String tabs1 =
		ParamUtil.getString(
			request, "tabs1", UserMgtUtil.TOP_TABS_WORKINGUNIT);
	PortletURL searchURL = renderResponse.createRenderURL();

	long workingUnitId =
		ParamUtil.getLong(
			request, EmployeeDisplayTerm.WORKING_UNIT_ID, 0L);

	List<WorkingUnit> workingUnits = new ArrayList<WorkingUnit>();

	try {
		workingUnits =
			WorkingUnitLocalServiceUtil.getWorkingUnits(scopeGroupId, 0);
			...
			<c:choose>
			     <c:when test="<%= tabs1.contentEquals(UserMgtUtil.TOP_TABS_EMPLOYEE)%>">
					...
					<c:when test="<%=tabs1.contentEquals(UserMgtUtil.TOP_TABS_WORKINGUNIT) %>">
			          <%
                        searchURL.setParameter("mvcPath", templatePath + "workingunitdirectory.jsp");
...
<%!private Log _log =
		LogFactoryUtil.getLog("html.portlets.usermgt.admin.toolbar.jsp");%>
