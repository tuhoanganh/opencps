...
public class FiveMinutes implements MessageListener{
    ...
    public void receive(Message message)
		...
		try {
			processWorkflows =
			    ProcessWorkflowLocalServiceUtil.getProcessWorkflowByEvent(SchedulerKeys.FIVE_MIUTELY);
			...
			for (ProcessWorkflow processWorkflow : processWorkflows) {
				...
				if (processOrders.size() != 0) {
					for (ProcessOrder processOrder : processOrders) {
						...
						_log.info("Scheduler _FIVE_MINUTELY ########" + processWorkflow.getActionName() + "_" + processOrder.getDossierId());
    ...
    private Log _log = LogFactoryUtil.getLog(FiveMinutes.class);
