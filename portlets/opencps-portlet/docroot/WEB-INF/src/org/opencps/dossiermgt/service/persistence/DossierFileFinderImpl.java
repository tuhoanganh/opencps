...
public class DossierFileFinderImpl extends BasePersistenceImpl<DossierFile>
	...
	public static final String COUNT_DOSSIER_FILE_ADVANCE = DossierFileFinder.class
			.getName() + ".countDossierFileAdvance";

	public static final String COUNT_DOSSIER_FILE_BY_G_K_T_R = DossierFileFinder.class
	...
	public static final String SEARCH_DOSSIER_FILE_ADVANCE = DossierFileFinder.class
			.getName() + ".searchDossierFileAdvance";

	public static final String SEARCH_DOSSIER_FILE_BY_G_K_T_R = DossierFileFinder.class
	...
	public static final String SEARCH_DOSSIER_FILE_RESULT = DossierFileFinder.class
			.getName() + ".searchDossierFileResult";
	
	public static final String SEARCH_DOSSIER_FILE_SUGGESTION = DossierFileFinder.class
			.getName() + ".searchDossierFileSugesstion";

	private Log _log = LogFactoryUtil.getLog(DossierFileFinder.class.getName());
	 ...
	 * @return
	 */
	private int countDossierFile(long groupId, long ownerUserId,
			long ownerOrganizationId, String[] keywords, String templateFileNo,
			int removed, boolean andOperator) {
		...
		try {
			...
			String sql = CustomSQLUtil.get(COUNT_DOSSIER_FILE_BY_G_K_T_R);
			...
			if (ownerUserId <= 0) {
				sql = StringUtil.replace(sql,
						"AND (opencps_dossier_file.ownerUserId = ?)",
						StringPool.BLANK);
			}
	...
	public int countDossierFileAdvance(long groupId, long ownerUserId,
			long ownerOrganizationId, String keyword, String templateFileNo,
			int removed, int partType, int original) {
		...
		return countDossierFileAdvance(groupId, ownerUserId,
				ownerOrganizationId, keywords, templateFileNo, removed,
				partType, original, andOperator);
	}
	 ...
	 * @return
	 */
	private int countDossierFileAdvance(long groupId, long ownerUserId,
			long ownerOrganizationId, String[] keywords, String templateFileNo,
			int removed, int partType, int original, boolean andOperator) {
		...
		try {
			...
			if (partType < 0) {
				sql = StringUtil.replace(sql,
						"AND opencps_dossierpart.dossierpartId = ?",
						StringPool.BLANK);
			}
	 ...
	 * @return
	 */
	private List<DossierFile> searchDossierFile(long groupId, long ownerUserId,
			long ownerOrganizationId, String[] keywords, String templateFileNo,
			int removed, int start, int end, OrderByComparator obc,
			boolean andOperator) {
		...
		try {
			...
			return (List<DossierFile>) QueryUtil.list(q, getDialect(), start,
					end);
		} catch (Exception e) {
			_log.error(e);
		} finally {
	 ...
	 */
	public List<DossierFile> searchDossierFileResult(int removed,
			long dossierId, int syncStatus, int dossierPartResultType,
			int dossierPartMultiResultType, int start, int end,
			OrderByComparator orderByComparator) throws SystemException {

		return _searchDossierFileResult(removed, dossierId, syncStatus,
				dossierPartResultType, dossierPartMultiResultType, start, end,
				orderByComparator);
	}
	 ...
	 * @throws SystemException
	 */
	private List<DossierFile> _searchDossierFileResult(int removed,
			long dossierId, int syncStatus, int dossierPartResultType,
			int dossierPartMultiResultType, int start, int end,
			OrderByComparator orderByComparator) throws SystemException {
	...
	public List searchDossierFileAdvance(long groupId, long ownerUserId,
		...
		return searchDossierFileAdvance(groupId, ownerUserId,
				ownerOrganizationId, keywords, templateFileNo, removed,
				partType, original, start, end, obc, andOperator);
	}
	 ...
	 * @return
	 */
	private List<DossierFileBean> searchDossierFileAdvance(long groupId,
			long ownerUserId, long ownerOrganizationId, String[] keywords,
			String templateFileNo, int removed, int partType, int original,
		...
		try {
			...
			if (itr.hasNext()) {
				while (itr.hasNext()) {
					...
					dossierFileBean.setDossierFileId(dossierFile
							.getDossierFileId());
	...
	public List<DossierFile> searchDossierFileSuggestion(
			long dossierId, String templateFileNos, long userId, int start, int end) throws SystemException {

		return _searchDossierFileSuggestion(dossierId, templateFileNos, userId, start, end);
	}
	
	private List<DossierFile> _searchDossierFileSuggestion(
			long dossierId, String templateFileNos, long userId, int start, int end)
			throws SystemException {
		...
		try {
			...
			String sql = CustomSQLUtil.get(SEARCH_DOSSIER_FILE_SUGGESTION);

			if (Validator.isNull(dossierId)) {
				sql = StringUtil.replace(sql, "AND odf.dossierId = ?",
						StringPool.BLANK);
			}
